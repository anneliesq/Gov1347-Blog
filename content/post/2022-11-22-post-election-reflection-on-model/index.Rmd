...
predictInterval1 <- data_frame(model=c("Dem"), predicted=c(predict1D[1,1],48.3), lower=predict1D[1,2], upper=predict1D[1,3], actual=c(1,0))

predictInterval3 <- data_frame(model=c("Dem"), predicted=c(predict3D[1,1], 48.3), lower=predict3D[1,2], upper=predict3D[1,3], actual=c(1,0))
...
plot3 <- ggplot(predictInterval3, aes(model, predicted, colour = factor(actual))) +        
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = lower, ymax = upper)) +
  scale_colour_manual(values= c("black", "purple"),labels=c('Predicted', 'Actual'), name=NULL) +
  ggtitle("Model 3")
