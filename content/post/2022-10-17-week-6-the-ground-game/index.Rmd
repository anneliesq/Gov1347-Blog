...
# Creating tibble for each district
train_data2 <- df %>% 
  left_join(nationalTR, by = "year") %>% 
  
  filter(year >= 1978) %>%
  # if district level TR unavilable, use national
...
# Using 2018 as test data with current economics and polling
test_data <- df %>%
  left_join(nationalTR, by = "year") %>% 
...
# Need to fix train data because has too many rows, so deleting duplicates
train_data <- test_data %>% 
  ...
  mutate(district_num = case_when(is.na(district_num) ~ 1,
                                  !is.na(district_num) ~ district_num)) %>% 
  mutate(avg_rating = case_when(is.na(avg_rating) ~ 1.000,
                                !is.na(avg_rating) ~ avg_rating))

districtRegression <- train_data %>%
  group_by(state, district_num) %>% 
  mutate(turnout = lag(turnout)) %>%
  select(state, district_num, year, turnout, ave_dem, incumbent, avg_rating, DemVotesMajorPercent) %>% 
  filter(year < 2018 )
# 
# 
# summary(districtRegression)
# 
# ahh <- districtRegression %>% 
#   filter(year == 2020)
...
districtTest <- test_data %>% 
  ...
  distinct(state, district_num, year, turnout, ave_dem, incumbent, avg_rating, partyMatch.y, DSPIC_change_pct, CPIAUCSL.y)
...
for (i in 1:50){
  for (j in 1:maxD){
   stateI <- states$state[i]
    ...
    if(nrow(test) != 0) {
      fit <- lm(DemVotesMajorPercent ~ incumbent + ave_dem + turnout + avg_rating, data = train)
...
seats <- mat %>% 
  mutate(demMajority = _when(V3 > 50 ~ 1,
                                 V3 <= 50 ~ 0)) %>% 
...
sum(seatsTR$demMajority, na.rm = T)
 mean(seatsTR$V4, na.rm = T)
